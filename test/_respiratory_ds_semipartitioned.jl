...
save_wav_path = "/home/paso/datasets/health_recognition/Respiratory_Sound_Database/audio_semipartitioned"
save_csv_path = "/home/paso/datasets/health_recognition/Respiratory_Sound_Database"
csv_name = "patient_diagnosis_semipartitioned"
...
csvdf = CSV.read(csv_file, DataFrame, header=header)
...
dff.group = map(x -> frag_func(x), dff.filename)
...
for i in grouped_dff
    ...
    push!(csv_df, hcat(frag_func(i.filename[1]), column_dict[parse(Int, split(frag_func(i.filename[1]), "_")[1])]))
    save_audio(Audio(audiodata, audioparams.sr), string(save_wav_path, "/", frag_func(i.filename[1]), ".wav"))
end
